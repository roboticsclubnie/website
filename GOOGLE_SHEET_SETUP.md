...
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({ 
      status: "active", 
      message: "Robotics Club NIE Recruitment Webhook is running and ready to receive submissions." 
    }))
...
function doPost(e) {
  try {
    ...
    // Fallback if payload was delivered via JSON
    if ((!p.email && !p.name) && e && e.postData && e.postData.contents) {
      try {
